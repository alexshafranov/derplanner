...
// Default allocator, uses `plnnr::allocate` and `plnnr::deallocate`.
...
// allocate an array of type `T` with compiler specified alignment of type `T`.
template <typename T>
inline T* allocate(Memory* mem, size_t count)
{
    return allocate<T>(mem, count, plnnr_alignof(T));
}
