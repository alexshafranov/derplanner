...
#ifndef DERPLANNER_COMPILER_ID_TABLE_INL_
#define DERPLANNER_COMPILER_ID_TABLE_INL_
...
namespace id_table
{
    ...
    template <typename T>
    bool _set(plnnrc::Id_Table<T>& table, const char* key, uint32_t length, uint32_t key_hash, const T& value)
    {
        const uint32_t max_size = table.max_size;
        uint32_t curr_hash = key_hash;
        ...
        for (uint32_t probe = 0; probe < max_size; ++probe)
        {
            ...
            const uint32_t slot_dist = id_table::probing_distance(slot_hash, slot, max_size);
            if (slot_dist < probe)
            {
                table.hashes[slot] = curr_hash;
                table.keys[slot] = curr_key;
    ...
    {
        const uint32_t key_hash = id_table::hash(key, length);
        return _set(table, key, length, key_hash, value);
    }
    ...
    inline void _grow(plnnrc::Id_Table<T>& table, uint32_t new_max_size)
    {
        ...
        {
            const uint32_t key_hash = old_hashes[i];
            ...
            if (key != 0)
            {
                _set(table, key, length, key_hash, value);
            }
        }
